...
## 7. 비용 차트 (Spending Charts)

> 출처: `src/components/dashboard/spending-charts.tsx`

| # | 요구사항 | 구현 상태 |
|---|---------|----------|
| 78 | 월별 지출 전망 컬럼 차트 (향후 6개월, 결제 주기 반영) | O |
| 79 | 연간 구독은 결제 기념월에만 반영, 주간은 4.33주 환산 | O |
| 80 | 카테고리별 월 지출 비율 가로 막대 차트 (비중 % 표시) | O |
| 81 | 차트 호버 툴팁 (금액, 구독 건수, 비중) | O |
| 82 | 다크모드 대응 (테마 토큰 기반 단일 색상) | O |
| 83 | 활성 구독 없을 때 빈 상태 안내 | O |

## 8. 검색 / 정렬 / 내보내기 (Search, Sort & Export)

> 출처: `src/components/dashboard/subscription-list.tsx`, `src/lib/export-csv.ts`

| # | 요구사항 | 구현 상태 |
|---|---------|----------|
| 84 | 서비스명/플랜/메모 텍스트 검색 (검색어 지우기 버튼 포함) | O |
| 85 | 검색 결과 건수 표시 + 결과 없음 빈 상태 | O |
| 86 | 정렬: 결제일 임박순 (기본) | O |
| 87 | 정렬: 금액 높은순/낮은순 (월 환산 기준) | O |
| 88 | 정렬: 이름순 (한국어 locale) / 최근 등록순 | O |
| 89 | CSV 내보내기 (현재 필터·정렬된 목록 기준) | O |
| 90 | CSV BOM 포함 — Excel에서 한글 깨짐 방지 | O |
